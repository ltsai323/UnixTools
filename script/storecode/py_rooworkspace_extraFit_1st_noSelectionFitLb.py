...
storeFileName='store_root/workspace_extraStep_1st_shortRangeLbFit_noKinematicCut_2ndPoly_noBsCorr.root'
storeFigName='store_fig/pdf_workspace_extraStep_1st_shortRangeLbFit_noKinematicCut_2ndPoly_noBsCorr.pdf'
...
if load2016Data:
    ...
    if LbModelBuilding:
        ...
        if buildSigModel:
            ...
            #space.factory('Gaussian::gPartPDF_{label}1({massVar},mu_{label},sig_{label}1)'.format(massVar=varName,label=mcLabel))
            #space.factory('Gaussian::gPartPDF_{label}2({massVar},mu_{label},sig_{label}2)'.format(massVar=varName,label=mcLabel))
            space.factory('Gaussian::gPartPDF_{label}1({massVar},mu_{label},sigma_MC_{label}1)'.format(massVar=varName,label=mcLabel))
            space.factory('Gaussian::gPartPDF_{label}2({massVar},mu_{label},sigma_MC_{label}2)'.format(massVar=varName,label=mcLabel))
        ...
        if buildBKGModel:
            ...
            #loadNumComb=space2nd.var(numCombName)
            space.factory('{name}[{initVal},{minVal},{maxVal}]'.format(name=numCombName,
                #initVal=loadNumComb.getVal(),minVal=0.,maxVal=dataset.sumEntries()*2.0))
